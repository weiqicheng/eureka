...
public class PeerAwareInstanceRegistryImpl extends AbstractInstanceRegistry implements PeerAwareInstanceRegistry {
    ...
    private void scheduleRenewalThresholdUpdateTask() {
        //读取配置：renewalThresholdUpdateIntervalMs，默认每15分钟调用一次
    ...
    public int syncUp() {
        ...
        /**
         * serverConfig.getRegistrySyncRetries()获取配置中的numberRegistrySyncRetries配置
         * 代表的是：当eureka服务器启动时尝试去获取集群里其他服务器上的注册信息的次数，默认为5
         * 设置了重试机制和等待机制。
         */
        for (int i = 0; ((i < serverConfig.getRegistrySyncRetries()) && (count == 0)); i++) {
            if (i > 0) {
                try {
                    /**
                     * serverConfig.getRegistrySyncRetryWaitMs() 获取配置中的registrySyncRetryWaitMs配置
                     * 代表的是：当eureka服务器启动时获取其他服务器的注册信息失败时，会再次尝试获取，期间需要等待的时间，默认为30s
                     */
            ...
            /**
             * 把当前server当成一个client，去相邻的eureka server节点，拷贝过来注册表
             */
    ...
    public void openForTraffic(ApplicationInfoManager applicationInfoManager, int count) {
        ...
        //初始化，更新期望一分钟要有多少次心跳的值：numberOfRenewsPerMinThreshold
    ...
    public boolean isLeaseExpirationEnabled() {
        /**
         * 是否启用了自我保护机制，读取配置enableSelfPreservation  默认是true
         * 如果关闭了自我保护机制，则此处永远返回true,就是说可以直接摘除掉实例。
         */
        ...
        /**
         * numberOfRenewsPerMinThreshold --> 期望的是一分钟要有多少次心跳发送过来
         * getNumOfRenewsInLastMin() --> 上一分钟所有服务实例一共发送过来多少次心跳
         * 如果上一分钟的心跳次数 大于 期望的值  那么就可以进行实例的故障清理 否则 不可以。
         */
